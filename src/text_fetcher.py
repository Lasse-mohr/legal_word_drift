...
from .xhtml_parser import parse_xhtml
from .utils.io import load_existing_ids, append_jsonl
from .utils.config import RAW_TEXTS_DIR
...
async def fetch_single_text(
    ...
    for lang in languages:
        ...
        if html:
            records = parse_xhtml(html.encode("utf-8"), celex)
            paragraphs = [r.text for r in records]
            paragraph_nums = [r.para_num for r in records]
...
async def fetch_texts_async(
    ...
    """
    processed = load_existing_ids(output_path, id_field="celex")
...
def fetch_texts(
    ...
    Args:
        celex_list: CELEX identifiers to fetch
        output_path: JSONL output path (default: data/raw/texts/texts.jsonl)
        concurrency: Max concurrent requests
        max_items: Limit (0 or None = all)
        languages: Language preference order (ISO 639-2/B codes)
    """
    output_path = output_path or os.path.join(RAW_TEXTS_DIR, "texts.jsonl")
